/* Create Insert Information for Department id and name */

INSERT INTO department (id, department_name) 
VALUES (111, 'Magic Spells'),
       (222, 'Evil Plans'),
       (333, 'World Domination'),
       (444, 'Forbidden Knowledge'),
       (555, 'Human Resources');


/* Create Insert Information for Role id, title, salary and department_id */

INSERT INTO role (id, title, salary, department_id)
VALUES (666, 'Mastermind', 500000, 222),
       (777, 'Wizard', 100000, 111),
       (888, 'Politician', 900000, 333),
       (999, 'Lich', 700000, 444),
       (000, 'Human Resources Employee', 60000, 555);

/* Create Insert Information for Employee id, first_name, last_name, role_id and manager_id */

INSERT INTO employee (id, first_name, last_name, role_id, manager_id)
VALUES (1, 'Agoth', 'the Terrible', 000, 1),
       (2, 'John', 'Rogers', 777, 2),
       (3, 'Kevin', 'Brown', 999, 3),
       (4, 'Lindsey', 'Jenkins', 666, 4),
       (5, 'Arcadia', 'Zelfor', 888, 5);